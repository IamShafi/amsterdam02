  ...
  return (
    <div>
      ...
      {/* {testimonialsLoading ? (
                  ...
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      {testimonials.filter(t => t.location && t.location.trim() !== "").slice(0, visibleReviews).map((testimonial, index) => <TestimonialCard key={index} {...testimonial} />)}
                    </div>
                    {visibleReviews < testimonials.filter(t => t.location && t.location.trim() !== "").length && (
                      <div className="flex justify-center mt-6">
                        <button
                          onClick={() => setVisibleReviews(prev => Math.min(prev + 4, testimonials.length))}
